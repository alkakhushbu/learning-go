...
func (c *Conn) CreateTask(ctx context.Context, newTask NewTask) (Task, error) {
...
func (c *Conn) GetTaskById(ctx context.Context, id int) (Task, error) {
...
func (c *Conn) GetAllTasks(ctx context.Context) ([]Task, error) {
...
// todo: update *AlterTask into AlterTask
func (c *Conn) UpdateTask(ctx context.Context, id int, alterTask AlterTask) (Task, error) {
	if alterTask.Name == "" && alterTask.Status == "" && alterTask.ManagedBy == "" {
		return Task{}, fmt.Errorf("all task field(s) empty: name, task status, and managedBy")
	}
	...
	if err != nil {
		return Task{}, fmt.Errorf("error in select query:%w", err)
	}
	slog.Info("Task", slog.Any("Existing task", dbTask))

	data, err := json.Marshal(alterTask)
	if err != nil {
		return Task{}, fmt.Errorf("error in marshaling data:%w", err)
	}

	err = json.Unmarshal(data, &dbTask)
	if err != nil {
		return Task{}, fmt.Errorf("error in unmarshaling data:%w", err)
	}
	...
	if err != nil {
		return Task{}, fmt.Errorf("error in update query:%w", err)
	}
	// time.Sleep(time.Second * 10)
	err = tx.Commit(ctx)
	if err != nil {
		return Task{}, fmt.Errorf("error in transaction commit:%w", err)
	}
	return dbTask, nil
}
